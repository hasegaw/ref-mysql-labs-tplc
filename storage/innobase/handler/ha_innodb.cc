...
static MYSQL_SYSVAR_ULONG(compression_algorithm, srv_compression_algorithm,
  PLUGIN_VAR_RQCMDARG,
  "0 for no compression, 1 for ZLib, 2 for LZ4. "
  "Note: ZLib & LZ4 use the level from --innodb-compression-level",
  NULL, NULL,
  0,		/* Default value - No compression */
  0,		/* Minimum value */
  2, 0);	/* Maximum value */

static MYSQL_SYSVAR_BOOL(compression_punch_hole, srv_punch_hole,
  PLUGIN_VAR_RQCMDARG,
  "Enable punch hole when PageIO compression is used (on by default)",
  NULL, NULL, TRUE);

static MYSQL_SYSVAR_BOOL(read_async, srv_read_async,
  PLUGIN_VAR_RQCMDARG,
  "Always do an ASYNC read from the buffer pool - defauly is OFF",
  NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(read_block_size, srv_read_block_size,
  PLUGIN_VAR_RQCMDARG,
  "Try and read only as much as required in file system block size units,"
  " instead of in InnoDB page size units",
  NULL, NULL, FALSE);
...
static struct st_mysql_sys_var* innobase_system_variables[]= {
  ...
  MYSQL_SYSVAR(compression_punch_hole),
  MYSQL_SYSVAR(read_async),

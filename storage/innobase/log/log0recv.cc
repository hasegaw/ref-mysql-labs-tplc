	...
	for (i = 0; i < n_hash_cells; i++) {
		...
		while (recv_addr != NULL) {
			...
			if (page_size.is_compressed()) {
				...
				error = fil_io(
					request, true,
					page_id, page_size, 0, page_size.bytes(),
					block->frame, NULL);
			}
	...
	lsn_t	max_flushed_lsn)/*!< in: max flushed lsn from data files */
{
	...
	IORequest	read_request(IORequest::READ | IORequest::LOG);

	fil_io(read_request, true, page_id, univ_page_size, 0,
	...
	if (0 == ut_memcmp(log_hdr_buf + LOG_FILE_WAS_CREATED_BY_HOT_BACKUP,
		...
		IORequest	write_request(IORequest::WRITE | IORequest::LOG);

		/* Write to the log file to wipe over the label */
		fil_io(write_request, true, page_id,
